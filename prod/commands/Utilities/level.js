...
function prepareLevelInfo(scoreOfUser) {
    ...
    const userLevelInfoFormat = `Level **${CurrentLevel}**    [ **${Math.round((scoreOfUser / CurrentLevelScore) * 100)}%** ]\n\n${scoreOfUser} out of ${CurrentLevelScore}`;
...
export default {
    ...
    async run(invokeParams) {
        ...
        const userToBeChecked = await mentionCheck(invokeParams.msg, invokeParams.author, invokeParams.args, invokeParams.isInteraction);
        ...
        try {
            const score = data != null
                ...
                : undefined;
            if (score != undefined) {
                const reply = await replier(invokeParams.msg, {

...
export default {
    ...
    async run(msg, args, author = msg.author, isInteraction = false) {
        if (!msg.member.permissions.has('MANAGE_GUILD'))
            return { content: 'You are lacking permission of: `Manage Server` =/' };
        let featureName;
        let decision;
        if (isInteraction) {
            ...
            if (!currentFeatures.includes(featureName))
                return { content: `**${featureName}** is not an available feature\nfeatures include: \`${currentFeatures.join(', ')}\`\n\n${cmdFormat}` };
            if (!validDecisions.includes(decision))
                return { content: `**${decision}** is not a valid option\nValid options: \`${validDecisions.join(', ')}\`\n\n${cmdFormat}` };
        }

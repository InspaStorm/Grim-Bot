...
export default {
  ...
  async run(invokeParams: CommandParamType) {
    ...
    const itemToBeUsed = invokeParams.msg.options.get("item")?.value as string;
  ...
  async handle(msg: ButtonInteraction | ModalSubmitInteraction) {
    ...
    if (msg.isButton()) {
      if (args[0] == YES_BUTTON) {
        ...
        await editReply(
          msg.message,
          { content: "Opening a text input..", components: [] },
          true
        );
    ...
    if (msg.isModalSubmit() && args[0] == MODAL_SUBMIT) {
      ...
      serverConf.singleUpdate(
        { guildId: msg.guild!.id },
        { $push: { custom_replies_list: newCustomReply } }
      );

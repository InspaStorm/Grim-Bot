import { ButtonInteraction, CommandInteraction, CommandInteractionOptionResolver, MessageActionRow, MessageButton, User } from "discord.js";
...
export default {
    ...
    async run(msg: CommandInteraction, args: CommandInteractionOptionResolver, author: User = msg.user, isInteraction: boolean = false) {
        ...
        const newGame: TicTacToe = tttLobby.addGame({msgId: gameMessage!.id, playerId: author.id, playerName: author.username, answer: null, components: gameButtons, endCallback: (msgID: string) => {
    ...
    async handle(inter: ButtonInteraction) {
        ...
        if(interactedGame) {
            ...
            const updatedButtons = interactedGame.updateBoard((interactedButton as MessageButton))

...
async function makeAwardEmbed(user: User) {
  ...
  const achievementEmbed = new EmbedBuilder()
    ...
    .setColor("#00ffff")
    .setFooter({
      text: `Found ${achievementIndexes.length} out of ${
...
export default {
  ...
  async run(invokOptions: CommandParamType) {
    let user = await inputMemberCheck(
      invokOptions.msg.guild!,
      invokOptions.author,
      invokOptions.args,
      invokOptions.isInteraction
    );
